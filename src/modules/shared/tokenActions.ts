...
import { EIP712Signature, TheaERC20Token, TheaNetwork } from "../../types";
...
export const permit = async (
	...
	switch (options.token) {
		case "ERC20":
			return new TheaERC20(signer, getERC20ContractAddress(options.tokenName, network)).permit(
				owner,
				options.spender,
				options.amount
			);
		case "ERC1155":
			return new TheaERC1155(signer, network).permit(owner, options.spender);
		default:
			throw new TheaError({ type: "NOT_SUPPORED_TOKEN_TYPE", message: "Token type does not exist" });
	}
};
